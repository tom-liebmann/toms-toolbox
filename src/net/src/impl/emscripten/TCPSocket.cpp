...
namespace
{
    ...
    {
    public:
        SocketDataWriter( ttb::emscripten::TCPSocket& socket );

        virtual size_t write( void const* data, size_t size ) override;

    private:
        ttb::emscripten::TCPSocket& m_socket;
    };
}
...
namespace ttb
{
    ...
    {
        return std::make_unique< emscripten::TCPSocket >( address, port );
    }
...
namespace
{
    ...
    SocketDataWriter::SocketDataWriter( ttb::emscripten::TCPSocket& socket ) : m_socket( socket )
    {
    }
