...
namespace ttb
{
    ...
    template < typename T >
    ttb::Vector< decltype( std::declval< T >() * std::declval< float >() ), 3 > operator*(
        ttb::Matrix4f const& matrix, ttb::Vector< T, 3 > const& vector );
...
namespace ttb
{
        ...
        ttb::Matrix4f const& matrix, ttb::Vector< T, 3 > const& vector )
    {
        ...
        return ttb::Vector< result_type, 3 >( { v[ 0 ], v[ 1 ], v[ 2 ] } );
    }
}
