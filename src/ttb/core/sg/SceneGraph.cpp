...
#include <ttb/core/sg/Instance.hpp>
#include <ttb/core/sg/LeafNode.hpp>
#include <ttb/core/sg/Modifier.hpp>

#include <algorithm>
#include <list>

using namespace ttb::sg;
    ...
    while( !nodeStack.empty() )
    {
        ...
        else
        {
            node->traverseModifier(
                [&modifierList, &modifierStack, &nodeStack]( const Modifier* modifier ) {
            ...
            {
                state->pushInstance(
                    new Instance( modifierList, static_cast< const LeafNode* >( node ) ) );
            }
            else
            {
                static_cast< const InnerNode* >( node )->traverseChildren(
                    [&nodeStack]( const Node* child ) { nodeStack.push_front( child ); } );
            }
        }
    }
...
void SceneGraph::State::pushInstance( Instance* instance )
{
    ...
    if( iter != m_passes.end() )
        ...
        m_passes.emplace( instance->getLeafNode()->getRenderPass(),
                          std::make_tuple( false, std::vector< Instance* >( { instance } ) ) );
}

namespace
{
    bool sortInstances( Instance* instance1, Instance* instance2 )
    {
...
void SceneGraph::State::render( ttb::State& state, uint32_t pass )
{
    ...
    if( iter != m_passes.end() )
    {
        ...
        for( auto& current : instances )
        {
            ...
            if( last )
            {
                ...
                while( cIter != current->getModifiers().end() &&
                       lIter != last->getModifiers().end() && ( *cIter ) == ( *lIter ) )
                {
                    ++cIter;
                    ++lIter;
                }
        ...
        if( last )
        {
            for( auto iter = last->getModifiers().rbegin(); iter != last->getModifiers().rend();
                 ++iter )
                ( *iter )->end( state );
        }
    }
